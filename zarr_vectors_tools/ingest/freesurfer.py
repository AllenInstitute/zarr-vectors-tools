...
store (layout in :mod:`~zarr_vectors_tools.ingest._surface_store`).
...
the footer carries it, so the store lines up with tracts and volumes of the
same subject.  Inflated and spherical surfaces are left alone: they are not
in any anatomical space, and shifting them means nothing.  ``c_ras`` is
    ...
    *,
    ...
    Args:
        ...
        hemispheres: ``lh``/``rh`` (or ``left``/``right``).  Default: both,
            whichever exist.
        space: ``"auto"`` (scanner RAS when the surfaces record ``c_ras``,
            else surface RAS), ``"scanner"`` (require ``c_ras``) or
            ``"surface"`` (leave FreeSurfer's coordinates untouched).

    Returns:
        The surface-store summary plus ``subject`` and ``c_ras``.
    ...
    if space == "scanner" and not complete:
        raise IngestError(
            ...
            "volume footer. Use space='surface' to keep FreeSurfer's own "
            "coordinates."
        )
