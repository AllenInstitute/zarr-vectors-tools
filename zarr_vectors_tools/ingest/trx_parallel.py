...
from zarr_vectors.constants import FRAGMENT_ATTRIBUTES, VERTEX_ATTRIBUTES, VERTICES
...
def _phase_b_worker(batch: dict[str, Any], shared: dict[str, Any]) -> npt.NDArray:
    ...
    # :func:`ingest_trx_parallel`).
    if coords_to_groups:
        for cc, vert_groups in coords_to_groups.items():
            write_chunk_vertices(
                level_group, cc, vert_groups, dtype=out_dtype,
                record_presence=False,
            )
            write_chunk_fragment_attributes(
                level_group, "segment_id", cc, coords_to_seg_ids[cc],
    ...
    *,
    ...
    try:
        ...
        with open_write_session(
            ...
            # predecessor->successor, matching trk_parallel's level-0 family.
            create_links_family(
        ...
        # Phase B writes every cell with record_presence=False, so
        # ``nonempty_chunks`` needs a coordinator-side repair once, from this
        # single process. For a sharded array the manifest can't be
        # re-derived from a store listing (a shard packs many cells into one
        # object), so repair it directly from the coordinator's own
        # already-known-correct occupied-chunk set; rebuild_nonempty_manifests
        # below then covers the unsharded case (and no-ops for the sharded
        # arrays just stamped).
        if level_shard_shape is not None:
            ...
            for name in dpv_names:
                stamp_nonempty_chunks_explicit(
                    level_group, f"{VERTEX_ATTRIBUTES}/{name}", _written_tccs,
                )
        ...
        try:
            if cross_chunk_link_count:
                ...
                # form write_link_cells expects — one shard's file at a
                # time (not concatenated up front) to keep peak memory
                # bounded to a single shard's link count rather than the
                # level's total. write_link_cells only touches the cells
                # its own records land in and does not maintain the
                # family-wide counts; finalize_links (below, after every
                # shard's file is written) reconciles them once.
        ...
        # multiscale_links marks the presence of delta != 0 (cross-LEVEL)
        # arrays only. Unconditional, not gated on cross_chunk_link_count:
        # create_links_family above already left the group meta readers
        # expect on an empty level, and finalize_links reconciles the
        # decentralized per-cell writes above once, whether or not any
        # landed.
