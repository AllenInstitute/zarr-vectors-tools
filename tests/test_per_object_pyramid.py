...
def _build_store(tmp_path, seed=0, n=20):
    ...
    write_polylines(
        ...
        # Route to the per-object binning coarsener (streamline geometry would
        # route to the RDP polyline coarsener instead — this file exercises the
        # per-object pyramid).
...
def test_object_attribute_present_mask_roundtrip(tmp_path):
    ...
    # Present rows have their source value; absent rows carry the dtype's
    # "absent" sentinel (NaN for float attributes) and are flagged by
    # present_mask == 0.
    for oid in range(20):
        if mask[oid]:
            assert out[oid] == oid
