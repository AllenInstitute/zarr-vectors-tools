...
For sweeps of zarr-vectors against *itself* (size, backend, chunk
shape, codec, pyramid depth, edit cost), see the sibling suite in
[`../internals/`](../internals/).
...
jupyter lab benchmarks/formats/
```
...
> `01_size_scaling.ipynb` (the only notebook here carrying them).
